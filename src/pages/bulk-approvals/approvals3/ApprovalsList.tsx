...
export default function ApprovalsList() {
  ...
  // The header's Reject/Approve always act on the single open document —
  // checkboxes never factor in here, so this is never a hidden override.
  const requestSingleDecision = (status: Exclude<ApprovalStatus, 'pending'>) => {
    if (!openDocument) return
    setPendingDecision({ status, documents: [{ id: openDocument.id, employeeName: openDocument.employeeName }] })
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', backgroundColor: colorPalette.white }}>
      <ApprovalHeader
        ...
        disabled={openDocument?.status !== 'pending'}
        selectedCount={0}
        onReject={() => requestSingleDecision('rejected')}
        onApprove={() => requestSingleDecision('approved')}
      />
